...
const CreateFilterDailog = ({ isOpen, isEdit, openStatus, onFilterSaved }) => {
  ...
  const [
    searchOptionsGroupBycategory,
    setSearchOptionsGroupBycategoryLabel,
  ] = React.useState([]);
  ...
  useEffect(() => {
    let searchOps = searchOptions
      ? searchOptions.filter((x) => !x.category)
      : [];
    let searchOptionsGroupBycategoryLabel = [];
    searchOptionsGroupBycategoryLabel = groupBy(
      [...searchOps],
      "categoryLabel"
    );
  ...
  function handleKeyChange(event, idx) {
    ...
    let newValue = JSON.parse(
      event?.target.options[event.target.selectedIndex].dataset.object
    );
    if (newValue) {
      const ageRow = [...age];
      ageRow[idx] = newValue.variable;
    ...
    if (i == 0) {
      return (
        ...
        [
          ...
          <MenuItem key={value.variable}>{value.title}</MenuItem>,
        ]
        // </>
      );
    } else {
      return <MenuItem key={value.variable}>{value.title}</MenuItem>;
    }
  }
      ...
      >
                            ...
                            <tr id="addr0" key={idx}>
                              ...
                              <td>
                                <TextField
                                  ...
                                  onChange={($event) =>
                                    handleChangeCriteria($event, idx)
                                  }
                                />
                              </td>
                              <td>
                            ...
                            <tr id="variable-row" key={idx}>
                              ...
                              <td>
                                <TextField
                                  ...
                                  onChange={($event) =>
                                    handleVariableChange($event, idx)
                                  }
                                />
                              </td>
                              <td>
                                <TextField
                                  id="variable-input-value"
                                  label="Value"
                                  value={variables[idx].value}
                                  onChange={($event) =>
                                    handleChangeVariables($event, idx)
                                  }
                                />
                              </td>
                              <td>
                                <IconButton aria-label="delete">
                                  <DeleteIcon
                                    id="delete-variable-row"
                                    onClick={(event) =>
                                      handleRemoveSpecificVariableRow(idx)
                                    }
                                  />
                                </IconButton>
                              </td>
        ...
        <DialogActions className={classes.dialogActions}>
          <Button
            ...
            className={classes.error + " " + (edit ? "" : classes.hide)}
          >
            Delete
          </Button>
