...
export default class CaseActivities extends Component {
  ...
  getCaseActivitiTaskList(activitiType = "") {
    if (this.state.caseInstanceId) {
      ...
      getCaseActivitiTasks(this.state.caseInstanceId, BASE_URL).then(
        (response) => {
          if (response.success) {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.caseInstanceId !== prevProps.caseInstanceId) {
      ...
      if (
        !this.state.caseActivitiTasks ||
        this.state.refreshPage ||
        (prevState.caseActivitiTasks &&
          this.state.caseInstanceId !== prevState.caseInstanceId)
      ) {
        this.getCaseActivitiTaskList();
